...
- Functions like `withdrawal`, `deposit`, and `transfer` perform the from the class `FundsContoller`, respective operations with error handling.
...
- So you can see there that there are three seperate objects based on each constructor. 
- We can control the independently. 
...
Now we can utilise the `pinCheck` function:
